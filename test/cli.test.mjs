...
test("buildLogViewerCommand uses the native terminal viewer script", async () => {
  ...
  assert.match(script, /clearScreenAndScrollback/);
  assert.match(script, /\\x1b\[3J/);
  assert.match(script, /"\\n"\.repeat\(terminalRows\(\)\)/);
  assert.match(script, /^\s*stdout\.write\(content\)/m);
  assert.match(script, /clearScreenAndScrollback\(\);\nstdout\.write\(content\);/);
  assert.doesNotMatch(script, /\\x1b\[A/);
  assert.doesNotMatch(script, /\\x1b\[B/);
  assert.doesNotMatch(script, /\\x1b\[5~/);
  assert.doesNotMatch(script, /\\x1b\[6~/);
  assert.doesNotMatch(script, /\\x1bOH/);
  assert.doesNotMatch(script, /\\x1bOF/);
  assert.doesNotMatch(script, /keyboardView/);
  assert.doesNotMatch(script, /scrollKeyboardView/);
  assert.doesNotMatch(script, /jumpKeyboardView/);
  assert.doesNotMatch(script, /\bfunction render\(/);
  assert.doesNotMatch(script, /pagerMode/);
  assert.doesNotMatch(script, /\\x1b\[\?1049h/);
  assert.doesNotMatch(script, /\\x1b\[\?1007h/);
  assert.doesNotMatch(script, /\\x1b\[\?1h/);
